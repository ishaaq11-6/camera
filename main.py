# ========================================================
# WHITExTRUSTED PYTHON BOT HOSTING PLATFORM v6.3 FINAL
...
# • Force Join @KINGxSPAM with Verify
# • AutoFile Logic Fixed (Manual + Forward with Scan Result)
...
def send_welcome(message):
    ...
    if not is_user_joined_channel(user_id):
        ...
        welcome = (
            "🚀 **WHITExTRUSTED Bot Hosting Platform v6.3 FINAL**\n\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            "🔹 24/7 Reliable Python Bot Hosting\n"
            "🔹 Advanced Malware & Bomber Protection\n"
            "🔹 Dynamic Bot Management Panel\n"
            "🔹 AutoFile Mode with Smart Scan\n"
        ...
        bot.send_message(message.chat.id, welcome, parse_mode="Markdown", reply_markup=force_join_keyboard())
        return
    ...
    bot.send_message(message.chat.id, welcome, parse_mode="Markdown", reply_markup=main_keyboard(is_admin))
...
def handle_upload(message):
    ...
    if AUTO_APPROVE_MODE:
        if clean:
            ...
            conn.commit()
            forward_text += "\n✅ **Auto Approved** (Clean file)"
        else:
            forward_text += "\n🚫 **Blocked** (Malware/Bomber detected)"
    else:
    ...
    if not AUTO_APPROVE_MODE or not clean:
        markup = types.InlineKeyboardMarkup(row_width=2)
        markup.add(
    ...
    bot.send_message(message.chat.id, "✅ File has been sent to admin for processing.")
...
def give_vip(message):
    ...
    try:
        ...
        try:
            bot.send_message(target_id, f"🎉 **Congratulations! VIP Activated**\nMax Bots: 15\nValid until: {expiry[:10]}", parse_mode="Markdown")
        except:
            pass
    except:
...
def toggle_autofile(message):
    ...
    status = "✅ ENABLED (Smart Scan + Forward)" if AUTO_APPROVE_MODE else "❌ DISABLED (Manual Approval)"
    bot.send_message(message.chat.id, f"🔄 **AutoFile Mode Updated**\n**Current Status**: {status}", parse_mode="Markdown")

# ========================= RUN THE BOT =========================
if __name__ == "__main__":
    print("=" * 100)
    print("🚀 WHITExTRUSTED PYTHON BOT HOSTING PLATFORM v6.3 FINAL")
    print("✅ All features working | Large Welcome | AutoFile fixed | Management Panel ready")
    print("=" * 100)

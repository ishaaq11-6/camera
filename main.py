# ========================================================
# WHITExTRUSTED PYTHON BOT HOSTING PLATFORM v6.5 FINAL
# Ultra Fancy • Professional • Secure • 24/7 Hosting
# ========================================================
# SABSE BADA & FULLY EXPANDED LARGE CODEBASE
# All features working perfectly with detailed comments
# AutoFile ON/OFF Logic exactly as requested
# Ban/Unban, VIP, AutoFile, Force Join, Management Panel all fixed
# ========================================================
...
# Bot start hone par missing packages automatically install karta hai
...
# Bot ke basic settings yahan hain. Inko change kar sakte ho.
...
OWNER_USERNAME = "@KINGxISHAAQ"                                 # Owner name as requested

CHANNEL_USERNAME = "@KINGxSPAM"                                 # Force Join Channel

bot = telebot.TeleBot(BOT_TOKEN)

AUTO_APPROVE_MODE = False                                       # Default OFF
USER_CPU_LIMIT = 50.0                                           # Normal users CPU limit
MAX_FILE_SIZE = 5 * 1024 * 1024                                # 5 MB maximum file size
IST = pytz.timezone('Asia/Kolkata')                             # Indian Standard Time

# ========================= DIRECTORIES & DATABASE =========================
# Bot ke data ko store karne ke liye folders aur database setup
...
# Files table - har uploaded bot ki information
...
# Users table - user details, ban status, VIP expiry etc.
...
running_processes = {}   # Currently running bots ko track karne ke liye

# ========================= SECURITY SCAN (Only Subprocess Related) =========================
# Sirf dangerous subprocess related code check karta hai. Bomber scan completely removed.
def perform_security_scan(file_path: str) -> tuple:
    """Security scan sirf subprocess, os.system, exec, eval jaise dangerous patterns ke liye"""
    try:
...
# ========================= CPU MONITOR =========================
# Background thread jo CPU usage monitor karta hai
def monitor_cpu_and_dead_processes():
    """CPU usage monitor. Admin unlimited, normal users 50% limit."""
    while True:
        try:
...
def send_welcome(message):
    ...
    if not is_user_joined_channel(user_id):
        ...
        text = (
            ...
            "🔹 Auto Package Installer for your bots\n"
...
# ========================= UPLOAD HANDLER (Final AutoFile Logic) =========================
...
def callback_handler(call):
    ...
    try:
        if data.startswith("manage_"):
            ...
            c.execute("SELECT filename, run_status, file_path FROM files WHERE id=?", (fid,))
            row = c.fetchone()
            if not row:
                return
            fname, rstatus, fpath = row
            is_running = rstatus == "running"

            if fpath and os.path.exists(fpath):
                with open(fpath, "r", encoding="utf-8", errors="ignore") as f:
                    content = f.read()[:3000]
                bot.send_message(call.message.chat.id, f"📄 **File Content**\n\n```{content}```", parse_mode="Markdown")
...
if __name__ == "__main__":
    print("=" * 100)
    print("🚀 WHITExTRUSTED PYTHON BOT HOSTING PLATFORM v6.5 FINAL")
    print("✅ All keyboard buttons working | All commands working | AutoFile fixed")
    print("✅ Ban/Unban, VIP, Force Join, Management Panel all working")
    print("=" * 100)

#!/usr/bin/env python3
# =============================================================================
#  STORM HOSTING BOT — Full Production SaaS System
...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8646183276:AAEyhwtKmJSzRxVdeIrNDUZ67bSsZP7Qcpo")
ADMIN_IDS_RAW = os.environ.get("ADMIN_IDS", "6026998790")
...
def cmd_start(message):
    ...
    welcome = (
        f"⚡ <b>Welcome to Ishaaq HOSTING{'!' if not is_new else ', ' + name + '!'}</b>\n\n"
...
def handle_broadcast_input(message):
    ...
    for u in users:
        ...
        result = safe_send(u["user_id"],
                           f"📢 <b>Announcement from ishaaq Hosting:</b>\n\n{broadcast_text}")
        if result:
            success += 1
        else:
            fail += 1
...
def handle_ban_input(message):
    ...
    safe_send(target_id, "🚫 You have been banned from Ishaaq Hosting.")
    clear_state(user_id)
...
def handle_unban_input(message):
    ...
    safe_send(target_id, "✅ You have been unbanned from ishaaq Hosting. Welcome back!")
    clear_state(user_id)
...
def handle_premium_user_id_input(message):
    ...
    if action == "add":
        ...
        safe_send(user_id, f"✅ Premium removed from user <code>{target_id}</code>.",
                  reply_markup=admin_menu())
        safe_send(target_id, "ℹ️ Your ishaaq Hosting premium has been removed.")
        clear_state(user_id)
...
def main():
    ...
    logger.info("=== ishaaq HOSTING BOT STARTING ===")
